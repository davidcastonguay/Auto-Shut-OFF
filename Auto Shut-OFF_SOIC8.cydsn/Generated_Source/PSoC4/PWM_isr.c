...
void PWM_isr_Stop(void)
{
    ...
    PWM_isr_SetVector(&IntDefaultHandler);
}
...
{
    #ifdef PWM_isr_INTERRUPT_INTERRUPT_CALLBACK
        PWM_isr_Interrupt_InterruptCallback();
    #endif /* PWM_isr_INTERRUPT_INTERRUPT_CALLBACK */ 
...
{
    CyRamVectors[CYINT_IRQ_BASE + PWM_isr__INTC_NUMBER] = address;
}
...
{
    return CyRamVectors[CYINT_IRQ_BASE + PWM_isr__INTC_NUMBER];
}
...
*
*   Note calling PWM_isr_Start or PWM_isr_StartEx will 
*   override any effect this API would have had. This API should only be called
*   after PWM_isr_Start or PWM_isr_StartEx has been called. 
...
void PWM_isr_SetPriority(uint8 priority)
{
    ...
    uint32 priorityOffset = ((PWM_isr__INTC_NUMBER % 4u) * 8u) + 6u;
    
	interruptState = CyEnterCriticalSection();
    *PWM_isr_INTC_PRIOR = (*PWM_isr_INTC_PRIOR & (uint32)(~PWM_isr__INTC_PRIOR_MASK)) |
...
uint8 PWM_isr_GetPriority(void)
{
    ...
    priority = (*PWM_isr_INTC_PRIOR & PWM_isr__INTC_PRIOR_MASK) >> priorityOffset;
...
uint8 PWM_isr_GetState(void)
{
    ...
    return ((*PWM_isr_INTC_SET_EN & (uint32)PWM_isr__INTC_MASK) != 0u) ? 1u:0u;
}
