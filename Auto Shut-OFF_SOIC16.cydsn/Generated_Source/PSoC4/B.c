...
#define SetP4PinDriveMode(shift, mode)  \
    do { \
        B_PC =   (B_PC & \
                                (uint32)(~(uint32)(B_DRIVE_MODE_IND_MASK << (B_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << (B_DRIVE_MODE_BITS * (shift))); \
    } while (0)
...
{
    uint8 drVal = (uint8)(B_DR & (uint8)(~B_MASK));
    drVal = (drVal | ((uint8)(value << B_SHIFT) & B_MASK));
    B_DR = (uint32)drVal;
}
...
{
	SetP4PinDriveMode(B__0__SHIFT, mode);
}
...
uint8 B_Read(void) 
{
    return (uint8)((B_PS & B_MASK) >> B_SHIFT);
}
...
{
    return (uint8)((B_DR & B_MASK) >> B_SHIFT);
}
...
/* If Interrupts Are Enabled for this Pins component */ 
    ...
    *******************************************************************************/
    ...
    {
		uint8 maskedStatus = (uint8)(B_INTSTAT & B_MASK);
		B_INTSTAT = maskedStatus;
        return maskedStatus >> B_SHIFT;
    }
